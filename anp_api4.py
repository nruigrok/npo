...
if __name__ == '__main__':
    ...
    logging.info(f"Scraping into AmCAT")
    ...
    for source in sources:
        chunks = get_chunks(api_calls(s, source), batch_size=args.batchsize)
        for batch in chunks:
            print(f"!!! Uploading {len(batch)} articles")
            print(batch)
    
  #  upload_batches(conn, index, tweets, platform=_PLATFORM, omroep=omroep['omroep'], update_fields=_UPDATEFIELDS)
