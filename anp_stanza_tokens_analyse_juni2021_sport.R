...
####TELETEKST
nieuws = amcat.getarticlemeta(conn, project=78, articleset = 3691, dateparts=T,time=T, columns = c("publisher", "date","title", "text", "section", "author"))

#tt2 = tt1 %>% mutate(text = str_c(title, text, sep="\n\n")) %>% select(doc_id=id, text=text)
#tt2 %>% write_csv("/tmp/teletekst.csv")

total=nieuws%>%filter(date>"2020-01-01")
...
npo_meta = npo_meta%>%left_join(nieuwstot)%>%select(-text)%>%mutate(date=as.POSIXct(date))
...
tot4=total%>%left_join(e_tot)
table(is.na(tot4$id))
tot4%>%select(from,id)%>%unique()%>%nrow()

saveRDS(tot4, "data/npo_juni2021_teletekst.rds")

head(tot4)

overlap = tot4%>%filter(! is.na(n_ngram))

saveRDS(overlap, "data/overlap_anp_npo_teletekst.rds")

overlap=readRDS("data/overlap_anp_npo_teletekst.rds")
...
head(tokens_npo)
table(tokens_npo$value, useNA = "always")



weight_zin = tokens_npo%>%mutate(value2=ifelse(is.na(value2),0,1))%>%group_by(doc_id, pb_id)%>%dplyr::summarise(n_woorden=sum(value2))%>%mutate(n_zinnen = n_woorden/10)%>%rename(id=doc_id)
table(weight_zin$n_woorden)


saveRDS(tokens_npo,"data/tokens_npo2021_teletekst.rds")
tot5 = tot4%>%rename(pb_id=from)%>%left_join(weight_zin)

saveRDS(tot5,"data/model_npo2021_teletekst.rds")
table(tot5$n_zinnen)
...
length(unique(overlap3$pb_id))
length(unique(overlap3$doc_id))

length(unique(overlap$pb_id))

length(unique(tot4$doc_id))
overlap %>% select(id, pb_id) %>% unique
