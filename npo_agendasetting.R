...
d = query("SELECT * FROM codingjobs WHERE project_id=320")
cjids = paste(d$codingjob_id, collapse=",")

sql = glue::glue("
SELECT ca.article_id, cf.label AS field, cf.fieldtype_id AS fieldtype, cl.label AS code, cv.intval, cv.strval,  * FROM 
  coded_articles ca 
    ...
    LEFT JOIN codebooks_codes cc ON cc.code_id=cl.code_id
    LEFT JOIN codes_labels cls ON cls.code_id = cc.parent_id 
WHERE codingjob_id IN ({cjids})
")

d = query(sql) 
# Drop duplicate coding_ids
dupes = d %>% select(coded_article_id, coding_id, sentence_id) %>% unique %>% group_by(coded_article_id, sentence_id) %>% filter(n()>1) %>% arrange(coded_article_id, sentence_id, desc(coding_id) ) %>% filter(row_number() > 1)
