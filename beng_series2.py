...
def get_items(model, id, item, offset=0, limit=10):
    ...
    data = api_call(f"/model/series/urn:vme:default:series:2101608030025065531/members?offset=2628&limit=10")
    print(data)
    keys = {x.replace("Pagination", "") for x in data.keys()} - {"id", "type"}
    print(keys)
    for key in keys:
        if "programs" in key:
            keys = set(key.replace("Pagination", ""))
            print(f"lengte keys={(keys)}")
            if len(keys) != 1:
                raise Exception(f"Cannot find members name from {keys}: {key}")
            key = list(keys)[0]
...
def scrape_series(serie: str, out_folder: Path):
    serie_id = parse_urn(serie)
    serie_title = get_series_title(serie)
    serie_title= re.sub(r"\W", "", serie_title)
    serie_complete = str(serie_id)+str(serie_title)
    folder = out_folder / serie_complete
    logging.info(f"Scraping serie {serie} to {folder}")
    if not folder.exists():
        logging.info(f"Creating output folder {folder}")
        folder.mkdir(parents=True)
    for program in get_all_items("series", serie, "members"):
        program_id = program
        ...
        try:
            data.update(scrape_program(program))
        except ItemNotFound:
...
if __name__ == '__main__':
    ...
    if urn.startswith("urn:vme:default:series:"):
        out_folder = Path.cwd() / "data" / "Nieuwsuur2018B"
...
#Nieuwsuur2018
#urn:vme:default:season:2101806250247979031
#2019
